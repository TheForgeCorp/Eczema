...
A personal eczema tracker for one user, self-hosted on a Beelink and used from an iPhone as an installed PWA. Not a clinical tool. The AI features suggest, they do not diagnose.
...
- Screens (see mockup): Today (feed), Meals (analyzer), Skin (severity + photo history), Trends (itch line with change-events marked), Insights (hypotheses), Dermatologist summary (6-month report), Reminders (settings).
...
- Insights: feed recent logs plus the symptom trend to the model and ask for candidate correlations as hypotheses, with 1 to 3 day lag in mind and confounds called out. Never conclusions.
